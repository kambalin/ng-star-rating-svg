import { ChangeDetectorRef, Component, ElementRef, forwardRef, Input, OnInit, Renderer2, ViewEncapsulation } from '@angular/core';
...
export const STRATE_VALUE_ACCESSOR: any = {
  ...
  useExisting: forwardRef(() => StratingComponent),
  multi: true
};

@Component({
  selector: 'nsrs-strating',
  template: `
  ...
  `,
  styleUrls: ['./strating.component.css'],
  encapsulation: ViewEncapsulation.None,
  providers: [STRATE_VALUE_ACCESSOR]
})
export class StratingComponent implements OnInit, ControlValueAccessor {
  ...
  protected settings = {
    ...
    strokeColor: 'black',
    starSize: 40,
    callback: function (rating: any, elem: any) { },
    onHover: function (index: any, rating: any) { },
    onLeave: function (index: any, rating: any) { }
  };
  _value: number = 0;
  ...
  getSvgDimensions(starShape: any) {
  ...
  executeCallback(rating: any, el: any) {
    var callback = this.settings.callback;
    callback(rating, el);
  }

  applyRating(rating: any) {
    var index = rating - 1;
  ...
  getOffsetByPixel(e: any, target: any, width: any) {    
    var leftOffset = this.elemOffset(target)!.left;
    var leftX = e.pageX - leftOffset;    
    return (leftX <= (width / 2) && !this.settings.useFullStars) ? 'left' : 'right';
  };
  
  paintStars(endIndex: any, stateClass: any) {    
    var polygonLeft;
    var polygonRight;
    var leftClass;
    var rightClass;
    this.stars.forEach((star: any, index: any) => {
